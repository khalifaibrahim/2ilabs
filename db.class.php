<?php
  /** A PHP class to access MySQL database with convenient methods
    ...
    *@Edited by Lukman Ibrahim (2dx.com.ng)
    */
  class DB
  {
   ...
   header("location: install.php?msg=$data");
    ...
    // get data from database
     function loginUser($table, $email, $password)
    {
        
      $queryUser ="SELECT * FROM `$table` WHERE email='$email' and password='$password'";
     // $result = mysqli_query($this->connection,$queryUser) or $this->debugAndDie($queryUser);
     // $count = mysqli_num_rows($result);
      return  $queryUser;

    }
      
      // post datainto database
      
        function addUser($table, $fullname, $email, $password)
    {
        
      $queryaddUser ="insert into `$table` (id, fullname, email, password) values ('', '$fullname', '$email', '$password' ) ";
    ...
    // get all questions from database
     function fetchQuiz($table)
    {
        
      $queryQuiz ="SELECT * FROM `$table`";
     // $result = mysqli_query($this->connection,$queryUser) or $this->debugAndDie($queryUser);
     // $count = mysqli_num_rows($result);
      return  $queryQuiz;
